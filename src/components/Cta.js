    ...
    <div className="w-full flex flex-col px-4 lg:px-10 py-10">
      <Slider ref={sliderRef} {...settings}>
        {testimonialData.map((testimonialData, index) => {
          return (
            <div key={index}>
              <TestimonialComp testimonialData={testimonialData} />
            </div>
          );
        })}
      </Slider>
      <div className="ml-auto flex flex-row gap-2">
        <CiCircleChevLeft
          className={clsx("circle-arrow", {
            "text-gray-300": slideIndex === 0,
            "text-blue-500": slideIndex !== 0,
          })}
          onClick={handlePrevChange}
        />
        <CiCircleChevRight
          className={clsx("circle-arrow", {
            "text-gray-300": slideIndex >= testimonialData.length - 1,
            "text-blue-500": slideIndex < testimonialData.length - 1,
          })}
